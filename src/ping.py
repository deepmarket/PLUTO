"""
Helper functions used for determining the state of a server.
"""
from os import getpid, devnull
from sys import platform, exc_info, stdout, stderr
from socket import gethostbyname, getaddrinfo, error
...
def ping_once(dest_addr, timeout=1):
    ...
    try:
        ...
        my_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM, ICMP_CODE)
...
def verbose_ping(dest_addr, timeout=2, count=4):
    ...
    for i in range(count):
        ...
        delay = ping_once(dest_addr, timeout)
        if delay == None:
...
class PingQuery(asyncore.dispatcher):
    def __init__(self, host_, p_id, timeout=0.5, ignore_errors=False):
        """
...
def multi_ping_query(hosts, timeout=1, step=512, ignore_errors=False):
    ...
    while host_list:
        ...
        for ip in host_list[:step]:  # select supports only a max of 512
            id_ += 1
            sock_list.append(PingQuery(ip, id_, timeout, ignore_errors))
            host_list.remove(ip)
...
# MAX_THREADS = 5
# queue = Queue(maxsize=0)  # maxsize=0 --> infinite items for some reason
#
# ips = ["www.google.com",
#        "www.facebook.com",
#        "www.microsoft.com",
...
#
#
# def pingr(thread_num: int, q: Queue):
#     """
#     Pings IP address
#     """
#     while True:
#         ip_addr = q.get()
#         print(f"Thread {thread_num}: Pinging {ip_addr}")
#
#         ret = Popen(f"ping -c 1 {ip_addr}", shell=True, stdout=PIPE, stderr=STDOUT)
#
#         ret_val = ret.stdout.read()
#         if ret:
#             print(f"{ip_addr}: is alive\n\t{ret_val}")
#         else:
#             print(f"{ip_addr}: did not respond\n\t{ret}: {ret_val}")
#
#         q.task_done()
#
#
...
#     worker = Thread(target=pingr, daemon=True, args=(i, queue))
