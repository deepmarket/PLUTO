...
class JobsAddView(JobsAddViewUI):
    ...
    def on_submit_clicked(self):
        ...
        cores: int = self.cores.text()
        memory: int = self.memory.text()
        source_files: str = self.source_file.text()
        input_files: str = self.input_file.text()
        expected_time: str = self.expect_time.text()
        price = "0.005"
